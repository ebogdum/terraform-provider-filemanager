...
// GetSSHConfigHost reads SSH config for a specific host.
// This is a simplified parser for common options.
...
// ParseSSHConfig parses ~/.ssh/config for a specific host alias.
func ParseSSHConfig(hostAlias string) (*SSHConfigHost, error) {
	home, err := os.UserHomeDir()
	if err != nil {
		return nil, fmt.Errorf("failed to get home directory: %w", err)
	}
	...
	// Use the alias as hostname if not specified
	if config.Hostname == "" {
		config.Hostname = hostAlias
	}
...
func matchHost(hostname, pattern string) bool {
	...
	// This is a simplified version - full glob matching would be more complex
